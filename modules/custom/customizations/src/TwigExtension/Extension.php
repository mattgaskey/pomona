...
class Extension extends Twig_Extension {
  ...
  public function getFilters() {
    return [
      ...
      // inject styles into WYSIWYG field for email templates
      new \Twig_SimpleFilter('style_injector', [$this, 'styleInjector']),
      new \Twig_SimpleFilter('real_url', [$this, 'realURL'])
    ];
  }
  ...
  public function getFunctions()
  {
    return [
        ...
        new \Twig_SimpleFunction('real_url', [$this, 'realURL']),
    ];
  }
  ...
  /**
   * Transform a URL to a redirect, if applicable.
   */
  public function realURL($url) {
    $alias = \Drupal::service('path.alias_manager')->getPathByAlias($url);
    if (!stristr($alias, '/node')) { //only continue processing internal references
      return $url;
    }
    ...
    if (isset($params['node'])) {
      ...
      if (isset($node->nid->value)) {
        if ($node->bundle() == 'redirect' && $node->hasField('field_link')) {
          $redirect = $node->field_link->uri;
          if (!empty($redirect)) {
            return $redirect; //return redirect URL instead
          }
        }
      }
    }

    return $url; //it's not connected to a node - return original URL
  }
