...
namespace Drupal\entity_select\Plugin\EntityReferenceSelection;
 ...
 * Advanced selection logic from https://www.drupal.org/project/sers
 *
 * @EntityReferenceSelection(
 *   id = "improved_entity_selection",
 *   label = @Translation("Improved Entity Selection"),
 *   group = "improved_entity_selection",
 * )
 */
class ImprovedEntitySelection extends DefaultSelection {
  ...
  protected $filterCharacters = array(
    "not" => ImprovedEntitySelection::FILTER_NOT,
    "end" => ImprovedEntitySelection::FILTER_END,
    "start" => ImprovedEntitySelection::FILTER_START,
  );
  ...
  public function getReferenceableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
    ...
    foreach ($entities as $entity_id => $entity) {
      ...
      $message = $entity->toUrl()->toString();
      $options[$bundle][$entity_id] = Html::escape($label) . ' <br>&nbsp;<small>' . Html::escape($message) . '</small>';
    }
  ...
  protected function extractFilters($match = NULL) {
    ...
    foreach ($matches as $partial) {
      ...
      switch ($firstchar) {
        case ImprovedEntitySelection::FILTER_NOT:
          $filters['not'][] = substr($partial, 1);
          break;

        case ImprovedEntitySelection::FILTER_START:
          $filters['start'][0] = substr($partial, 1);
          break;

        case ImprovedEntitySelection::FILTER_END:
